...
### 3-1. 검색부터 신청 이후까지 연결하는 통합 워크플로우
...
| --- | --- |
| **서비스명** | GovBiz |
| **서비스 정의** | 기업에 적합한 정부지원사업의 탐색부터 신청 준비와 협업까지 연결하는 AI 기반 플랫폼 |
| **주요 사용자** | 정부지원사업을 찾고 신청하려는 기업 및 사업 담당자 |
| **해결 과제** | 여러 기관에 분산된 공고 탐색, 복잡한 신청 조건 확인, 신청 일정과 진행 상황 관리 |
| **검색 방식** | 기업 상황을 자연어로 입력하는 AI 대화 검색과 지역·분야·지원 대상 등을 선택하는 필터 검색 |
| **핵심 기능** | 통합 검색, 근거 기반 공고 분석, 기업 맞춤 리포트, 관심 공고 관리, 신청 문서 작성, 중복 지원·수혜 검토, 파트너 관리 |
| **데이터 출처** | 기업마당, K-Startup, 과학기술정보통신부, 충청남도 온라인수출지원시스템의 공공데이터포털 Open API |
| **신뢰성 원칙** | AI 답변만 제시하지 않고 공식 공고 원문과 확인 가능한 근거를 함께 제공 |
| **핵심 흐름** | 지원사업 탐색 → 근거 및 신청 조건 확인 → 관심 공고 저장 → 신청 준비 및 진행 관리 → 중복 검토 및 파트너 협업 |
| **서비스 구성** | React 프론트엔드, Spring Boot Core API, FastAPI AI Service, MySQL 및 검색·비동기 처리 인프라 |
| **배포 구성** | Vercel과 AWS 기반 배포 구조 |
